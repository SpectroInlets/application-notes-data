...
if True: # plot and fit the HER QC
    ...
    if False: #use current to find decay edges instead of HER signal, requires 
        #adapting tspan in the for loop below, so that the ca. 3s delay are taken
        #into account
        signal_tlist = "raw current / [mA]"
        t_list_clean = find_decay_edge(her, signal_tlist, gradient_cutoff=0.01)  
    t_half_h2_list = []        
